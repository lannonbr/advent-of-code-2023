...
fn run_instructions(network: Network) -> u64 {
    let end: String = String::from("ZZZ");

    let mut steps_taken = 0;

    let mut curr_location = network.map.get("AAA").unwrap();
...
fn main() {
    ...
    println!("Steps taken: {}", run_instructions(network));
}
